...
[source,bash]
----
export RELEASE_NAME="<your-solution-release-name>"
export NAMESPACE="<your-namespace>"
----
_Replace the placeholders with the values used in your environment._
...
To install the application, depending on which option best suits the required installation type, execute the following commands:

==== New install (Option A)

_Use the same `[flags]` as your standard AP solution install._

. Install secrets, CRDs, and infra as per your standard AP installation procedure.
+
*Example (secrets):*
+
[source,bash]
----
helm upgrade -i $RELEASE_NAME-secrets <SECRETS_CHART_FOLDER> -n $NAMESPACE [flags]
----
*Example (infra):*
+
[source,bash]
----
helm upgrade -i $RELEASE_NAME-infra <INFRA_CHART_FOLDER> -n $NAMESPACE [flags]
----

. Retrieve the Keycloak client secret from the cluster:
+
[source,bash]
----
CURRENT=$(kubectl get secret altiplano-secrets -n $NAMESPACE -o jsonpath='{.data.keycloak_client_secret}' | base64 --decode)
----
+
This reads `keycloak_client_secret` from the `altiplano-secrets` Kubernetes secret and stores it in the variable `$CURRENT` for use in the next step.

. Install the solution chart with ACCM enabled: ACCM needs database `app_cm` (user `app_cm`) on `altiplano-mariadb`. Create it and grant privileges if it does not exist yet.
+
[source,bash]
----
helm upgrade -i $RELEASE_NAME <SOLUTION_CHART_FOLDER> -n $NAMESPACE [flags] \
    --set global.ACCM_KEYCLOAK_PORTAL_SECRET=$CURRENT \
    --set tags.altiplano-accm=true \
    --set altiplano-accm.enabled=true
----

==== Existing setup (Option B)

. Confirm that the base Altiplano pods are healthy.
...
. Upgrade the existing solution release: ACCM needs database `app_cm` (user `app_cm`) on `altiplano-mariadb`. Create it and grant privileges if it does not exist yet.
+
[source,bash]
----
helm upgrade $RELEASE_NAME <SOLUTION_CHART_FOLDER> -n $NAMESPACE [flags] \
    --set global.ACCM_KEYCLOAK_PORTAL_SECRET=$CURRENT \
    --set tags.altiplano-accm=true \
    --set altiplano-accm.enabled=true
----
+
Replace `[flags]` with `--reuse-values` to keep your current configuration and only enable ACCM on top.
+
*Example:*
+
[source,bash]
----
helm upgrade $RELEASE_NAME <SOLUTION_CHART_FOLDER> -n $NAMESPACE --reuse-values \
    --set global.ACCM_KEYCLOAK_PORTAL_SECRET=$CURRENT \
    --set tags.altiplano-accm=true \
    --set altiplano-accm.enabled=true
----
+
[NOTE]
Replace `[flags]` with your usual install options.

Wait for the installation to be completed. To check all the required ACCM pods and services are up and running, execute the following command:
